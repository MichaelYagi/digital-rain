...
test('layers configure({bgColor}) updates container background', async ({ page }) => {
    await load(page);
    await page.evaluate(() => {
        window._rain = new DigitalRain('#container', {
            layers: [{ fontSize: 9, introDepth: 0 }, { fontSize: 14, introDepth: 0 }],
    ...
    await page.waitForTimeout(BOOT_MS);
    await page.evaluate(() => window._rain.configure({ bgColor: '#ff0000' }));
    const bg = await page.evaluate(() => document.querySelector('#container').style.backgroundColor);
    expect(bg).toBe('rgb(255, 0, 0)');
});

test('layers pause event fires from parent', async ({ page }) => {
    await load(page);
    await page.evaluate(() => {
        window._rain = new DigitalRain('#container', {
            layers: [{ fontSize: 9, introDepth: 0 }, { fontSize: 14, introDepth: 0 }],
...
test('layers resume event fires from parent', async ({ page }) => {
    await load(page);
    await page.evaluate(() => {
        window._rain = new DigitalRain('#container', {
            layers: [{ fontSize: 9, introDepth: 0 }, { fontSize: 14, introDepth: 0 }],
...
test('layers randomize() keeps same direction on all layers', async ({ page }) => {
    await load(page);
    await page.evaluate(() => {
        window._rain = new DigitalRain('#container', {
            direction: 'up',
...
test('layers hideChildren does not hide wrapper divs', async ({ page }) => {
    await load(page);
    await page.evaluate(() => {
        window._rain = new DigitalRain('#container', {
            hideChildren: true,
    ...
    const wrappersHidden = await page.evaluate(() =>
        Array.from(document.querySelectorAll('#container > div')).some(d => d.style.visibility === 'hidden')
    );
    expect(wrappersHidden).toBe(false);
});